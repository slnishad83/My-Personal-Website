...
(function() {
  ...
  // â”€â”€ Wait for app to be ready â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  function waitFor(check, cb) {
    ...
    // H8: Event-driven only â€” no polling fallback. Warn after 10s if not ready.
    const timeout = setTimeout(() => {
      if (!check()) if (window.__DEBUG__) console.warn('[FeaturesAddon] App not ready after 10s â€” features may not load');
    }, 10000);
  ...
  // â”€â”€ 2. FEATURE NAVIGATION BAR â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ 3. BUSY STATUS (Feature 10) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  function setupBusyStatus() {
    ...
    auth.onAuthStateChanged(user => {
      ...
      _busyStatusUnsub = db.collection('users').doc(user.uid).onSnapshot(snap => {
        ...
        if (window.__DEBUG__) console.error('[FeaturesAddon] Busy status snapshot error:', err?.message || err);
        if (window.App && typeof window.App.safeOnSnapshot === 'function') {
          setTimeout(function () {
            _busyStatusUnsub = db.collection('users').doc(user.uid).onSnapshot(function (snap2) {
              showOrHideBusyBanner((snap2.data() || {}).busyStatus);
            }, function () {});
          }, 3000);
        }
      });
    ...
    window.saveBusyStatus = async () => {
      ...
      try {
        ...
        if (window.__DEBUG__) console.error('[FeaturesAddon] saveBusyStatus write failed:', err);
    ...
    window.clearBusyStatus = async () => {
      ...
      try {
        ...
        if (window.__DEBUG__) console.error('[FeaturesAddon] clearBusyStatus write failed:', err);
  ...
  function showOrHideBusyBanner(busyStatus) {
    ...
    banner.innerHTML = `ðŸ”´ Busy: "${window.sanitizeHTML(busyStatus)}" â€” Auto-reply is on <button data-action="clearBusyStatus">Clear</button>`;
    const nav = document.getElementById('featureNavBar');
    if (nav && nav.parentNode) nav.parentNode.insertBefore(banner, nav.nextSibling);
    else document.body.prepend(banner);
  }

  // â”€â”€ 4. TIME CAPSULE (Feature 2) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  function setupTimeCapsuleButton() {
    ...
    window.saveTimeCapsule = async () => {
      ...
      try {
        ...
        modal.classList.remove('show');
        if (typeof showToast === 'function') showToast(`â³ Message scheduled for ${dueAt.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}`);
      } catch (err) {
        if (window.__DEBUG__) console.error('[FeaturesAddon] saveTimeCapsule write failed:', err);
        if (typeof showToast === 'function') showToast('Failed to schedule. Please try again.', 'error');
      }
    };
  }

  // â”€â”€ 5. CATCH ME UP AI (Feature 4) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  function setupCatchMeUpButton() {
    function _injectCatchUpBtn() {
      ...
      btn.title = 'Catch Me Up â€” AI summary of recent messages';
    ...
    window.catchMeUp = async () => {
      ...
      result.innerHTML = '<div class="catchup-title">ðŸ§  AI Summary</div><div>Analysing messagesâ€¦</div>';
      ...
      try {
        ...
        result.innerHTML = '<div class="catchup-title">ðŸ§  Catch Me Up</div>Could not generate summary. Try again.';
      }
    };
  }

  // â”€â”€ 6. TASKS PANEL (Feature 5) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  function setupTasksButton() {
    ...
    window.openTasksPanel = () => {
      ...
      tasksUnsubscribe = db.collection('tasks').where('userId', '==', user.uid).orderBy('createdAt', 'desc').limit(100).onSnapshot(snap => {
        ...
        if (window.__DEBUG__) console.error('[FeaturesAddon] Tasks snapshot error:', err?.message || err);
    ...
    window.addTask = async () => {
      ...
      try {
        ...
        if (window.__DEBUG__) console.error('[FeaturesAddon] addTask write failed:', err);
    ...
    window.toggleTask = async (id, done) => {
      try {
        ...
        if (window.__DEBUG__) console.error('[FeaturesAddon] toggleTask write failed:', err);
      }
    };
    window.deleteTask = async (id) => {
      try {
        ...
        if (window.__DEBUG__) console.error('[FeaturesAddon] deleteTask write failed:', err);
      }
    };
    ...
    function renderTasks(tasks) {
      ...
      el.innerHTML = tasks.map(t => `
        <div class="task-item">
          ...
          <button class="task-del" data-action="deleteTask" data-action-arg="${t.id}">ðŸ—‘</button>
        </div>`).join('');
    }
  }

  // â”€â”€ 7. VOICE TRANSCRIPTION (Feature 1) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  function setupVoiceTranscription() {
    ...
    window.transcribeVoice = async (msgId, btn, _bubble) => {
      ...
      try {
        ...
        if (!msgData?.attachment?.url) { btn.textContent = 'âŒ No audio'; return; }
        // Check if already transcribed
        if (msgData.transcription) {
          const span = document.createElement('span');
          span.className = 'transcription-text';
          span.textContent = 'ðŸ“ ' + msgData.transcription;
        ...
        const span = document.createElement('span');
        span.className = 'transcription-text';
        span.textContent = 'ðŸ“ ' + (res.data.text || 'Could not transcribe');
        btn.replaceWith(span);
        // Save to Firestore
        ...
        btn.disabled = false;
        setTimeout(() => { btn.textContent = 'ðŸ“ Transcribe'; }, 2000);
      }
    };
  }

  // â”€â”€ 8. AUTO-TRANSLATE PREFERENCE (Feature 6) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  function setupAutoTranslatePreference() {
    ...
    window.setAutoTranslate = (enabled) => {
      ...
      if (typeof showToast === 'function') showToast(enabled ? 'ðŸŒ Auto-translate ON' : 'Auto-translate OFF');
    };
  }
