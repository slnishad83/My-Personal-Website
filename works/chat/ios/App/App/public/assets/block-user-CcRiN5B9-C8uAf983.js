...
<button class="blocked-user-unblock">Unblock</button>`,(function(t,n){d.querySelector(`.blocked-user-unblock`).addEventListener(`click`,function(){window.unblockUser(t,n),e.classList.remove(`open`),setTimeout(function(){e.remove()},200),setTimeout(function(){window.showBlockedUsersList()},300)})})(i[a],l),r.appendChild(d)}requestAnimationFrame(function(){e.classList.add(`open`)}),n.querySelector(`.blocked-users-close`).addEventListener(`click`,function(){e.classList.remove(`open`),setTimeout(function(){e.remove()},200)}),e.addEventListener(`click`,function(t){t.target===e&&(e.classList.remove(`open`),setTimeout(function(){e.remove()},200))})};function u(){var e=document.getElementById(`chat-header-name`)||document.getElementById(`chat-user-name`);if(e){var t=e.parentElement.querySelector(`.blocked-indicator`);if(t&&t.remove(),typeof currentChat<`u`&&currentChat&&currentChat.id&&window.isUserBlocked(currentChat.id)){var n=document.createElement(`div`);n.className=`blocked-indicator`,n.textContent=`ðŸš« Blocked`,e.parentElement.appendChild(n)}}}function d(){o();for(var e=document.querySelectorAll(`.chat-list-item[data-chat-id]`),t=0;t<e.length;t++){var n=e[t].dataset.chatId;window.isUserBlocked(n)&&(e[t].style.display=`none`)}}function f(){s(),a().then(function(){d(),u()})}document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,f):f()})()})),i=r()}))();export{i as default};