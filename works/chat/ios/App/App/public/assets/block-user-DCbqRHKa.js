...
`+(l?`<img class="block-dialog-avatar" src="`+n(l)+`" alt="" onerror="this.style.display='none'">`:`<div class="block-dialog-avatar" style="display:flex;align-items:center;justify-content:center;font-size:28px;">ðŸ‘¤</div>`)+`<div class="block-dialog-name">`+n(a||`Unknown`)+`</div>
...
`+(u?`<img class="blocked-user-avatar" src="`+n(u)+`" alt="" onerror="this.style.display='none'">`:`<div class="blocked-user-avatar" style="display:flex;align-items:center;justify-content:center;font-size:18px;">ðŸ‘¤</div>`)+`<div class="blocked-user-info">
  <div class="blocked-user-name">`+n(c)+`</div>
  <div class="blocked-user-sub">Blocked</div>
</div>
<button class="blocked-user-unblock">Unblock</button>`,(function(t,n){d.querySelector(`.blocked-user-unblock`).addEventListener(`click`,function(){window.unblockUser(t,n),e.classList.remove(`open`),setTimeout(function(){e.remove()},200),setTimeout(function(){window.showBlockedUsersList()},300)})})(i[a],c),r.appendChild(d)}requestAnimationFrame(function(){e.classList.add(`open`)}),t.querySelector(`.blocked-users-close`).addEventListener(`click`,function(){e.classList.remove(`open`),setTimeout(function(){e.remove()},200)}),e.addEventListener(`click`,function(t){t.target===e&&(e.classList.remove(`open`),setTimeout(function(){e.remove()},200))})};function d(){var e=document.getElementById(`chat-header-name`)||document.getElementById(`chat-user-name`);if(e){var t=e.parentElement.querySelector(`.blocked-indicator`);if(t&&t.remove(),typeof currentChat<`u`&&currentChat&&currentChat.id&&window.isUserBlocked(currentChat.id)){var n=document.createElement(`div`);n.className=`blocked-indicator`,n.textContent=`ðŸš« Blocked`,e.parentElement.appendChild(n)}}}function f(){s();for(var e=document.querySelectorAll(`.chat-list-item[data-chat-id]`),t=0;t<e.length;t++){var n=e[t].dataset.chatId;window.isUserBlocked(n)&&(e[t].style.display=`none`)}}function p(){c(),o().then(function(){f(),d()})}document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,p):p()})()}));export default t();