/**
 * In-App Help & Support
 * FAQ sections, contact support, and version info.
 */
(function () {
  'use strict';
  ...
  const HelpSupport = {
    _faqs: [
      {
        ...
        a: 'Tap the "+" button on the chat list, select "New Group", add members, set a group name, and tap Create.'
      },
      {
        q: 'How do I lock a chat?',
        a: 'Long-press any chat in the chat list and select "Lock Chat". You can set a PIN or use biometric authentication.'
      },
      {
        q: 'How do disappearing messages work?',
        a: 'Open a chat, tap the chat name at the top, and enable "Disappearing Messages". Messages will auto-delete after the selected timer.'
      },
      {
        q: 'Can I use NSL Chat on multiple devices?',
        a: 'Yes! NSL Chat works on Web (PWA), Android, iOS, and Desktop (Electron). Your chats sync across all devices in real-time.'
      },
      {
        q: 'How do I share my location?',
        a: 'Open a chat, tap the attachment icon (📎), and select "Location". You can share a static pin or start live location sharing.'
      },
      {
        q: 'How do I make a voice or video call?',
        a: 'Open a chat and tap the phone or video icon in the chat header. Group calls support up to 4 participants.'
      },
      {
        q: 'How do I backup my chats?',
        a: 'Go to Profile & Settings > chat settings > Export Chat. You can export as text, HTML, or JSON format.'
      },
      {
        q: 'How do I mute notifications?',
        a: 'Long-press a chat and select "Mute", or open chat info and toggle mute. You can mute for 8 hours, 1 week, or always.'
      },
      {
        q: 'What is two-step verification?',
        a: 'Two-step verification adds a PIN after your password for extra security. Enable it in Profile & Settings > Two-Step Verification.'
      },
      {
        q: 'How do I change my theme?',
        a: 'Go to Profile & Settings > App Theme to switch between Light, Dark, and System themes.'
      },
      {
        q: 'How do view-once media work?',
        a: 'When sending a photo or video, toggle "View Once" before sending. The recipient can only view it once before it disappears.'
      },
      {
        q: 'How do I search for messages?',
        a: 'Tap the search icon in any chat or use Ctrl+F / Cmd+F to search across all messages with filters.'
      },
    ],
    ...
    open() {
      ...
      panel.style.cssText = 'background:var(--surface-container,#1e1e2e);border-radius:20px;max-width:480px;width:92vw;max-height:85vh;overflow-y:auto;color:var(--on-surface;display:flex;flex-direction:column)';

      const faqHtml = this._faqs.map((faq, i) => `
        <div class="faq-item" style="border-bottom:1px solid var(--outline-variant,rgba(0,0,0,0.06))">
          <button class="faq-toggle" data-idx="${i}" style="width:100%;padding:14px 16px;border:none;background:transparent;color:var(--on-surface);text-align:left;font-size:13px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:space-between">
            <span>${faq.q}</span>
          ...
          <div class="faq-answer" data-idx="${i}" style="display:none;padding:0 16px 14px;font-size:12px;color:var(--on-surface-variant);line-height:1.6">${faq.a}</div>
        </div>`).join('');
      ...
      panel.querySelectorAll('.faq-toggle').forEach(btn => {
        btn.addEventListener('click', () => {
          const idx = btn.dataset.idx;
          const answer = panel.querySelector(`.faq-answer[data-idx="${idx}"]`);
      ...
      document.getElementById('help-report-bug')?.addEventListener('click', () => {
        overlay.remove();
        this._openBugReport();
      });
    ...
    _openBugReport() {
      ...
      panel.innerHTML = `
        ...
        <button id="bug-submit" style="width:100%;padding:12px;border-radius:10px;border:none;background:var(--primary);color:var(--on-primary);font-size:13px;font-weight:600;cursor:pointer;margin-top:12px">Submit Report</button>`;
      ...
      document.getElementById('bug-submit')?.addEventListener('click', async () => {
        ...
        if (!desc) { showToast('Please describe the bug', 'error'); return; }
        try {
          await App.db.collection('userReports').add({
            type: 'bug',
            description: desc,
            email: email || '',
    ...
    _openContactSupport() {
      ...
      panel.innerHTML = `
        ...
        <button id="support-submit" style="width:100%;padding:12px;border-radius:10px;border:none;background:var(--primary);color:var(--on-primary);font-size:13px;font-weight:600;cursor:pointer;margin-top:12px">Send Message</button>`;

      overlay.appendChild(panel);
      overlay.addEventListener('click', (e) => { if (e.target === overlay) overlay.remove(); });
      document.body.appendChild(overlay);

      document.getElementById('support-submit')?.addEventListener('click', async () => {
