...
// Idle timeout script
const idleScript = `/* C5: Auto-logout on 20min idle + C11: resetAppState on signout */
(function() {
...
function processIndexHtml() {
  ...
  // Update CSP to remove unsafe-inline/unsafe-eval for scripts
  html = html.replace(
    /<meta http-equiv="Content-Security-Policy" content="[^"]*">/,
    `<meta http-equiv="Content-Security-Policy" content="default-src 'self' https: blob: data: 'unsafe-inline'; script-src 'self' https://www.gstatic.com 'wasm-unsafe-eval'; style-src 'self' https://fonts.googleapis.com 'unsafe-inline'; img-src 'self' https: blob: data:; font-src 'self' https://fonts.gstatic.com; connect-src 'self' https://*.googleapis.com https://*.firebaseio.com https://*.firebaseapp.com https://*.cloudfunctions.net wss://*.firebaseio.com; frame-src 'self' https://*.firebaseapp.com; worker-src 'self' blob:; frame-ancestors 'self'; form-action 'self';">`
  );
