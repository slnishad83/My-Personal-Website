  ...
  function patchDesktopContextMenu() {
    ...
    document.addEventListener('contextmenu', function (e) {
      ...
      _fetchMsg(msgId).then(function (msg) {
        ...
        if (_chat && _chat.ephemeralTimer && _chat.ephemeralTimer > 0) {
          var _isKept = msg.kept === true || msg.keepInChat === true;
          html += '<button class="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl hover:bg-surface-variant/50 transition-colors text-on-surface" onclick="window.toggleKeepInChat(\'' + _esc(chat.id) + '\',\'' + _esc(msgId) + '\',' + (!_isKept ? 'true' : 'false') + ');document.getElementById(\'msg-ctx-menu\').remove()"><span class="material-symbols-outlined text-lg">' + (_isKept ? 'bookmark_remove' : 'bookmark_add') + '</span><span class="text-sm">' + (_isKept ? 'Keep in chat (on)' : 'Keep in chat') + '</span></button>';
        }
