import{n as e}from"./firebase-config-B1gHZycV.js";var t=e((()=>{(function(){let e=null,t=`nsl_disappearing_default`,n={getGlobalDefault(){try{return parseInt(localStorage.getItem(t)||`0`,10)}catch{return 0}},setGlobalDefault(e){try{localStorage.setItem(t,e.toString())}catch{}},applyDefaultToNewChats(e){let t=this.getGlobalDefault();if(t>0&&App.db&&App.auth?.currentUser){let n=(App.chats||[]).find(t=>t.id===e);if(n&&!n.ephemeralTimer){let r=n.type===`group`?`groups`:`chats`;App.db.collection(r).doc(e).update({ephemeralTimer:t}).catch(()=>{})}}},openGlobalDefaultSettings(){let e=this.getGlobalDefault(),t=`
    ...
    `;document.body.insertAdjacentHTML(`beforeend`,t)}window.saveDisappearingTimer=async function(){if(!window.App||!window.App.currentChat||!window.App.db)return;let e=document.querySelector(`input[name="ephemeral-timer"]:checked`);if(!e)return;let t=parseInt(e.value,10),n=window.App.currentChat.id;try{window.App.currentChat.type===`group`?await window.App.db.collection(`groups`).doc(n).update({ephemeralTimer:t}):await window.App.db.collection(`chats`).doc(n).update({ephemeralTimer:t}),window.App.currentChat.ephemeralTimer=t,o(),window.showToast&&window.showToast(`Disappearing messages timer updated`,`success`),document.getElementById(`disappearing-modal`)?.remove(),window.App.messages&&window.App.messages[n]&&window.sendMessage}catch(e){window.__DEBUG__&&console.error(e),window.showToast&&window.showToast(`Failed to update timer`,`error`)}};function o(){if(!window.App||!window.App.currentChat)return;let e=window.App.currentChat,t=document.getElementById(`header-ephemeral-icon`);if(e.ephemeralTimer&&e.ephemeralTimer>0){if(!t){let e=document.getElementById(`chat-header-info`);if(e){let t=document.createElement(`span`);t.id=`header-ephemeral-icon`,t.className=`material-symbols-outlined text-[16px] text-primary ml-2`,t.textContent=`timer`,t.title=`Disappearing Messages On`,e.appendChild(t)}}}else t&&t.remove()}function s(){if(!window.App||!window.App.messages||!window.App.db)return;let e=Date.now();(Array.isArray(window.App?.chats)?window.App.chats:Object.values(window.App?.chats||{})).forEach(t=>{if(!t||!t.id)return;let n=t.ephemeralTimer||0,r=t.id;if(n&&n>0){let i=window.App.messages[r];(Array.isArray(i)?i:[]).forEach(i=>{if(!(!i||i.kept===!0||i.keepInChat===!0)){var a=i.time||(i.timestamp&&i.timestamp.toMillis?i.timestamp.toMillis():typeof i.timestamp==`number`?i.timestamp:0);if(a&&e-a>n){let e=t.type===`group`||t.isGroup===!0?`groups`:`chats`;window.App.db.collection(e).doc(r).collection(`messages`).doc(i.id).delete().then(()=>{window.App.messages[r]&&(window.App.messages[r]=window.App.messages[r].filter(e=>e.id!==i.id))}).catch(e=>{e.code===7||e.message?.includes(`PERMISSION_DENIED`)?window.__DEBUG__&&console.warn(`[SelfDestruct] Delete blocked by rules, server will handle`):window.__DEBUG__&&console.error(`[SelfDestruct] Delete failed:`,e)})}}})}})}document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,r):r(),window.isMessageKept=function(e){return!!e&&(e.kept===!0||e.keepInChat===!0)},window.toggleKeepInChat=async function(e,t,n){if(!window.App||!window.App.db||!window.App.currentChat)return!1;try{let r=window.App.currentChat,i=r.type===`group`||r.isGroup===!0?`groups`:`chats`,a=window.App.auth&&window.App.auth.currentUser&&window.App.auth.currentUser.uid||window.currentUser&&window.currentUser.uid||``,o=window.App.db.FieldValue||window.firebase&&window.firebase.firestore&&window.firebase.firestore.FieldValue,s={};n?(s.kept=!0,s.keptAt=new Date,a&&(s.keptBy=o?o.arrayUnion(a):[a])):(s.kept=!1,o&&(s.keptAt=o.delete(),s.keptBy=o.delete())),await window.App.db.collection(i).doc(e).collection(`messages`).doc(t).update(s);let c=window.App.messages[e];if(Array.isArray(c)){let e=c.find(e=>e.id===t);e&&(e.kept=n,e.keepInChat=n)}return window.showToast&&window.showToast(n?`Message kept in chat`:`Message no longer kept`,`success`),!0}catch(e){return window.__DEBUG__&&console.error(`[SelfDestruct] toggleKeepInChat failed:`,e),window.showToast&&window.showToast(`Failed to update keep status`,`error`),!1}}})()}));export default t();