import{n as e}from"./modulepreload-polyfill-C_LrRQgL.js";import{$ as t,A as n,B as r,D as i,Dt as a,E as o,G as s,H as c,I as l,J as u,N as d,O as f,P as p,R as m,S as h,T as g,U as _,W as v,Y as y,Z as b,_ as x,_t as S,bt as C,ct as w,d as T,dt as E,et as D,g as O,h as k,ht as A,it as j,k as M,m as N,ot as P,p as F,r as I,rt as L,s as R,st as z,t as B,tt as V,ut as H,wt as U,x as W,xt as G,y as K,yt as q}from"./feature-security-DWxqWVD5.js";var J,Y;e((()=>{U(),u(),a((()=>{j(),w(),H((()=>{r(),c(),t((()=>{L(),m(),s((()=>{n(),A(),_((()=>{G(),V(),D((()=>{v(),l(),z((()=>{C(),E(),p((()=>{R(),P(),q((()=>{S(),d(),W((()=>{x(),y(),h((()=>{k(),B(),b((()=>{T(),K(),O((()=>{I(),M(),N((()=>{o(),i(),g((()=>{f(),J=F((()=>{(function(){let e=[],t=null;function n(){let e=document.createElement(`style`);e.textContent=`
      .saved-msg-item {
        padding: 12px 16px;
    ...
    `,n.innerHTML=`<span style="font-size:16px">ðŸ”–</span> Save Message`,n.onmouseenter=()=>n.style.background=`var(--surface-container-highest)`,n.onmouseleave=()=>n.style.background=`transparent`,n.onclick=()=>{window._removeCtxMenu&&window._removeCtxMenu(),i(t)};let r=Array.from(e.querySelectorAll(`button`)).find(e=>e.innerHTML.includes(`Delete`));r?e.insertBefore(n,r):e.appendChild(n)}async function i(e){if(!window.App||!window.App.db||!window.App.auth.currentUser)return;let t=window.App.auth.currentUser.uid,n=window.App.currentChat?.id;if(!n)return;let r=(window.App.messages[n]||[]).find(t=>t.id===e);if(r)try{await window.App.db.collection(`users`).doc(t).collection(`savedMessages`).doc(e).set({...r,savedAt:Date.now(),originalChatId:n,originalChatName:window.App.currentChat.name}),window.showToast&&window.showToast(`Message saved to Bookmarks`,`success`)}catch(e){window.__DEBUG__&&console.error(`Error saving message:`,e),window.showToast&&window.showToast(`Failed to save message`,`error`)}}function a(){if(t&&(t(),t=null),!window.App||!window.App.db||!window.App.auth.currentUser)return;let n=window.App.auth.currentUser.uid;t=window.App.db.collection(`users`).doc(n).collection(`savedMessages`).orderBy(`savedAt`,`desc`).onSnapshot(t=>{e=t.docs.map(e=>e.data()),window.App.activeTab===`more`&&o()})}function o(){let t=document.getElementById(`chat-list`);if(!t)return;if(document.getElementById(`chats-sidebar-title`).textContent=`Saved Messages`,e.length===0){t.innerHTML=`
      ...
      `;return}let n=``;e.forEach(e=>{let t=new Date(e.savedAt),r=t.toLocaleDateString()+` `+t.toLocaleTimeString([],{hour:`2-digit`,minute:`2-digit`}),i=e.text||``;(e.type===`image`||e.attachment?.type===`image`)&&(i=`ðŸ“· Image `+i),(e.type===`video`||e.attachment?.type===`video`)&&(i=`ðŸŽ¥ Video `+i),(e.type===`voice`||e.attachment?.type===`voice`)&&(i=`ðŸŽ¤ Voice message`),n+=`
      ...
      `}),t.innerHTML=n}window.openSavedMessage=function(e,t){window.selectChat&&(window.selectChat(e),setTimeout(()=>{window.scrollToMsg&&window.scrollToMsg(t)},500))},document.readyState===`loading`?(document.addEventListener(`DOMContentLoaded`,n),document.addEventListener(`nsl:app-ready`,a)):(n(),a())})()})),Y=J()}))()}))()}))()}))()}))()}))()}))()}))()}))()}))()}))()}))()}))()}))()}))()}))();export{Y as default};